...
*/
	...
	_consolePrintf("RD=%d ",filesysRootDir);
	_consolePrintf("RE=%d ",bootSec->rootEntries);
	_consolePrintf("REsize=%d\n",sizeof(DIR_ENT));
	_consolePrintf("SPC=%d ",bootSec->sectorsPerCluster);
	_consolePrintf("FD=%d ",filesysData);
	_consolePrintf("FLC=%d\n",fatLastCluster);

	if (fatLastCluster < 4085)
	{
		_consolePrintf("Detect FAT12\n",0);
		filesysType = FS_FAT12;	// FAT12 volume - unsupported
	}
	else if (fatLastCluster < 65525)
	{
		_consolePrintf("Detect FAT16\n",0);
		filesysType = FS_FAT16;	// FAT16 volume
	}
	else
	{
		_consolePrintf("Detect FAT32\n",0);
