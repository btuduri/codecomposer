...
ALIGNED_VAR_IN_DTCM TRCP RCP;
TRCP_Chank RCP_Chank;
...
void RCP_Free(void)
{
  ...
  for(u32 trk=0;trk<RCPTrackMax;trk++){
    if(pRCP->RCP_Track[trk].pLoop!=NULL){
      safefree(pRCP->RCP_Track[trk].pLoop); pRCP->RCP_Track[trk].pLoop=NULL;
    }
    if(pRCP->RCP_Track[trk].pExc!=NULL){
      safefree(pRCP->RCP_Track[trk].pExc); pRCP->RCP_Track[trk].pExc=NULL;
    }
  }
...
/*
  ...
  if(pRCP->pRCP_Tracks!=NULL){
    safefree(pRCP->pRCP_Tracks); pRCP->pRCP_Tracks=NULL;
  }
*/
...
static void RCP_LoadTrackChank(TRCP_Track *pRCP_Track)
{
  ...
  pRCP_Track->DataTop=&RCP.File[RCP.FilePos];
...
*/
  ...
  pRCP_Track->DataEnd=&RCP.File[RCP.FilePos+pRCP_Track->unuseTrackLen-RCP_TrackHeaderSize];
  ...
  pRCP_Track->pLoop=(TRCP_Track_Loop*)safemalloc(sizeof(TRCP_Track_Loop)*RCP_Track_LoopMax);
  ...
  pRCP_Track->pExc=(TRCP_Track_Exc*)safemalloc(sizeof(TRCP_Track_Exc));
...
static void RCP_Proc_LoopStart(TRCP_Track *pRCP_Track)
{
  if((pRCP_Track->LoopFreeIndex+1)==RCP_Track_LoopMax){
    _consolePrintf("Loop nest buffer overflow.\n");
    return;
  }
...
static void RCP_Proc_LoopEnd(TRCP_Track *pRCP_Track,u32 LoopCount)
{
  ...
  if(pRCP_Track->LoopFreeIndex==0){
    _consolePrintf("Loop nest not found.\n");
    return;
  }
...
static void RCP_ProcTempo(u32 _TempoFactor)
{
  ...
  u32 CurTempo=RCP_Chank.Tempo*pRCP->TempoFactor/64;
  
  if(CurTempo==0){
    _consolePrintf("Tempo setting error.\n");
  ...
  spc=(pRCP->SampleRate*0x10000)/RCP_Chank.TimeRes;
  spc=spc*60/CurTempo;
...
void RCP_LoadRCP(u8 *FilePtr,u32 SampleRate)
{
  TRCP *pRCP=&RCP;
...
static void ExcSend(bool ShowMessage,u32 ch,TRCP_Track_Exc *pExc)
{
  ...
  if((data[0]!=0x41)||(data[1]!=0x10)||(data[2]!=0x42)||(data[3]!=0x12)){
    if(ShowMessage==true) _consolePrintf("not support pExc-> format.\n");
    return;
  }
  
  if(data[4]!=0x40){
    if(ShowMessage==true) _consolePrintf("not support !send pExc-> format.\n");
    return;
  }
...
void RCP_ProcRCP(bool ShowMessage,bool EnableNote,TRCP_Track *pRCP_Track)
{
...
{
  return(RCP.SamplePerClockFix16);
}
