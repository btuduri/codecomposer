...
//extern const u8 powtbl_bin_end[];
//extern const u8 powtbl_bin[];
//extern const u32 powtbl_bin_size;

//static u16 *powtbl=(u16*)powtbl_bin;
#define POWRANGE (1024)

#include "sintbl_bin.h"
//extern const u8 sintbl_bin_end[];
//extern const u8 sintbl_bin[];
//extern const u32 sintbl_bin_size;
...
static void PCH_SetProgramMap_Load(TProgramMap *pPrgMap,int FileHandle,u32 FileOffset)
{
  ...
  fseek(pPrgMap->FileHandle,pPrgMap->FileOffset,SEEK_SET);
  
  fread(pPrgMap->DataOffset,128,4,pPrgMap->FileHandle);
...
void PCH_SetProgramMap(int FileHandle)
{
  ...
  fread(&ID,1,4,FileHandle);
  
  for(int prg=0;prg<128;prg++){
    fread(&ToneOffset[prg],1,4,FileHandle);
  }
  for(int prg=0;prg<128;prg++){
    fread(&DrumOffset[prg],1,4,FileHandle);
  }
  
  fread(&PCMFileOffset,1,4,FileHandle);
...
//    _consolePrintf("%d,%d,%d,%d,%d\n",patidx,NoteFreq>>16,NearFreq>>16,RootFreq>>16,NearPatchIndex);
  ...
  if(NearPatchIndex==0x100){
    _consolePrintf("find near root freq from patch list??\n");
    return(NULL);
  }
...
bool PCH_LoadProgram(s32 Note,u32 var,u32 prg,bool DrumMode)
{
  ...
  fseek(pPrgMap->FileHandle,pPrgMap->FileOffset+ofs,SEEK_SET);
  
  fread(&patcount,4,1,pPrgMap->FileHandle);
  ...
  for(u32 patidx=0;patidx<patcount;patidx++){
    TProgram *pPrg=&pPrgPatch->pPrg[patidx];
    fread(pPrg,1,ProgramPatchHeaderSize,pPrgMap->FileHandle);
...
/*
    _consolePrintf("EnvRate=");
    for(int idx=0;idx<6;idx++){
      _consolePrintf("%02x,",pPrg->EnvRate[idx]);
    }
    _consolePrintf("\n");
    
    _consolePrintf("EnvOfs =");
    for(int idx=0;idx<6;idx++){
...
*/
  ...
  for(u32 patidx=0;patidx<patcount;patidx++){
    ...
    fseek(pPrgMap->FileHandle,PCMFileOffset+pPrg->PCMOffset,SEEK_SET);
    
#define WAVEID (0x45564157)
#define TTACID (0x43415454)

    u32 ID;
    fread(&ID,1,4,pPrgMap->FileHandle);
    
    switch(ID){
      case WAVEID: {
        u32 datasize;
        fread(&datasize,1,4,pPrgMap->FileHandle);
        ...
        fread(pPrg->pData,1,datasize,pPrgMap->FileHandle);
      } break;
      case TTACID: {
        u32 CodeBufSize;
        u8 *pCodeBuf;
        
        fread(&CodeBufSize,1,4,pPrgMap->FileHandle);
        ...
        fread(&SourceSamplesCount,1,4,pPrgMap->FileHandle);
        
        fread(pCodeBuf,1,CodeBufSize,pPrgMap->FileHandle);
...
//          _consolePrintf("DecodeTTAC8bit. %dbytes %dsamples\n",CodeBufSize,SourceSamplesCount);
...
//          _consolePrintf("DecodeTTAC16bit. %dbytes %dsamples\n",CodeBufSize,SourceSamplesCount);
...
//          TTAC_Decode_12bit_asm(pCodeBuf,pSourceSamples,SourceSamplesCount);
  ...
  if(memoryoverflow==true){
    MemoryOverflowFlag=true;
    _consolePrintf("PatchLoad out of memory!!\n");
/*
    for(u32 patidx=0;patidx<patcount;patidx++){
      TProgram *pPrg=&pPrgPatch->pPrg[patidx];
      if(pPrg->pData!=NULL){
...
{
  for(u32 ch=0;ch<PCHCountMax;ch++){
    TPCH1 *_pPCH1=&PCH1[ch];
    TPCH2 *_pPCH2=&PCH2[ch];
