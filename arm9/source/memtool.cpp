...
{memchk{ if(v==NULL){ iprintf("Hooked memory address error.%s%d\n",__FILE__,__LINE__); while(1); } }
...
{memchk{ if((src==NULL)||(dst==NULL)){ iprintf("Hooked memory address error.%s%d\n",__FILE__,__LINE__); while(1); } }
  if(len==0) return;
...
{memchk{ if(dst==NULL){ iprintf("Hooked memory address error.%s%d\n",__FILE__,__LINE__); while(1); } }
  len/=2;
...
{
  iprintf("Please start with the A button if you use M3.\n");
}
...
void safefree(const void *ptr)
{
  ...
  if(ptr==NULL){
    iprintf("safefree Request NullPointer.\n");
    return;
  }
...
u32 PrintFreeMem(void)
{
  ...
  for(i=4*1024*1024;i!=0;i-=PrintFreeMem_Seg){
