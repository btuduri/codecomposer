...
TGlobalINI GlobalINI; // possible error : we did not initialize this structure!
TMIDIINI MIDIINI;
...
static void readsection(char *str)
{
  ...
  while(*str!=']'){
    if((128<=ofs)||(*str==0)){
      iprintf("line%d error.\nThe section name doesn't end correctly.\n",readline);

//	  	// This function cannot be define nor make by ourselves.
    }
    section[ofs]=*str;
    str++;
    ofs++;
...
//  iprintf("key=%s value=%s\n",key,value);
}

static void internal_LoadMIDIINI(char *pini,u32 inisize)
{
  section[0]=0;
  readline=0;
...
void LoadINI(char *data,int size)
{
  ...
  internal_LoadMIDIINI(data,size);
}
