...
static u32 M3_GetMemorySize(void)
{
  ...
  iprintf("PSRAM memory size %dMByte.\n",memsize/1024/1024);
...
void extmem_Init(void)
{
  ...
  switch(pInfo->Type){
    case EEMT_Unknown: {
      iprintf("extmem:unknown cartridge type.\n");
    } break;
    case EEMT_M3: {
      iprintf("extmem:set cartridge type to M3.\n");
    ...
    case EEMT_SC: {
      iprintf("extmem:set cartridge type to SC.\n");
    ...
    case EEMT_EZ4: {
      iprintf("extmem:set cartridge type to EZ4.\n");
    ...
    case EEMT_DSBM: {
      iprintf("extmem:set cartridge type to DSBM.\n");
...
void extmem_SetCount(u32 Count)
{
  ...
  switch(pInfo->Type){
    ...
    case EEMT_M3: {
      ...
      if(pBody->pSlot!=NULL){
        iprintf("extmem_SetSlotCount: Already allocated pSlot.\n");
        // ShowLogHalt();
      }
  ...
  switch(pInfo->Type){
    ...
    case EEMT_M3: {
      ...
      if(pBody->SlotCount<=SlotIndex){
        iprintf("extmem_Exists(u32 SlotIndex=%d); SlotCount==%d limit error.\n",SlotIndex,pBody->SlotCount);
        return(false);
      }
...
//  iprintf("extmem_Alloc(%d,%d);\n",SlotIndex,Size);
  ...
  switch(pInfo->Type){
    ...
    case EEMT_M3: {
      ...
      if(pBody->SlotCount<=SlotIndex){
        iprintf("extmem_Alloc(u32 SlotIndex=%d,u32 Size=%d); SlotCount==%d limit error.\n",SlotIndex,Size,pBody->SlotCount);
        return(false);
      }
...
//        iprintf("extmem_Alloc(u32 SlotIndex=%d,u32 Size=%d); MemAlloc limit error.\n",SlotIndex,Size);
        return(false);
      }
...
bool extmem_Write(u32 SlotIndex,void *pData,u32 DataSize)
{
  ...
  switch(pInfo->Type){
    ...
    case EEMT_M3: {
      ...
      if(pBody->SlotCount<=SlotIndex){
        iprintf("extmem_Write(u32 SlotIndex=%d,...); SlotCount==%d limit error.\n",SlotIndex,pBody->SlotCount);
        return(false);
      }
      ...
      if(pDstData==NULL){
        iprintf("extmem_Write(u32 SlotIndex=%d,...); pSlot[SlotIndex]==NULL not allocated error.\n",SlotIndex);
        return(false);
      }
...
bool extmem_Read(u32 SlotIndex,void *pData,u32 DataSize)
{
  ...
  switch(pInfo->Type){
    ...
    case EEMT_M3: {
      ...
      if(pBody->SlotCount<=SlotIndex){
        iprintf("extmem_Read(u32 SlotIndex=%d,...); SlotCount==%d limit error.\n",SlotIndex,pBody->SlotCount);
        return(false);
      }
...
//      u32 SrcSize=(Size+(PacketSize_M3-1)) & (~(PacketSize_M3-1));
      ...
      if(pSrcData==NULL){
        iprintf("extmem_Read(u32 SlotIndex=%d,...); pSlot[SlotIndex]==NULL not allocated error.\n",SlotIndex);
        return(false);
      }
