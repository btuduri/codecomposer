...
void PCH_SetProgramMap_Load(TProgramMap *pPrgMap, int FileHandle,u32 FileOffset)
{
  ...
  fseek((FILE*)(pPrgMap->FileHandle),pPrgMap->FileOffset,SEEK_SET);
  
  fread(pPrgMap->DataOffset,128,4,(FILE*)(pPrgMap->FileHandle));
...
void PCH_SetProgramMap(int FileHandle)
{
  ...
  fseek((FILE*)FileHandle,0x0,SEEK_SET);
  fread(&ID,1,4,(FILE*)FileHandle);
  
  for(prg=0;prg<128;prg++){
    fread(&ToneOffset[prg],1,4,(FILE*)FileHandle);
  }
  for(prg=0;prg<128;prg++){
    fread(&DrumOffset[prg],1,4,(FILE*)FileHandle);
  }
  
  fread(&PCMFileOffset,1,4,(FILE*)FileHandle);
...
bool PCH_LoadProgram(s32 Note,u32 var,u32 prg,bool DrumMode)
{
  ...
  fseek((FILE*)(pPrgMap->FileHandle),pPrgMap->FileOffset+ofs,SEEK_SET);
  
  fread(&patcount,4,1,(FILE*)(pPrgMap->FileHandle));
  ...
  for(patidx=0;patidx<patcount;patidx++){
    TProgram *pPrg=&pPrgPatch->pPrg[patidx];
    fread(pPrg,1,ProgramPatchHeaderSize,(FILE*)(pPrgMap->FileHandle));
  ...
  for(patidx=0;patidx<patcount;patidx++){
    ...
    fseek((FILE*)(pPrgMap->FileHandle),PCMFileOffset+pPrg->PCMOffset,SEEK_SET);
    
#define WAVEID (0x45564157)
#define TTACID (0x43415454)

    u32 ID;
    fread(&ID,1,4,(FILE*)(pPrgMap->FileHandle));
    
    switch(ID){
      case WAVEID: {
        u32 datasize;
        fread(&datasize,1,4,(FILE*)(pPrgMap->FileHandle));
        ...
        fread(pPrg->pData,1,datasize,(FILE*)(pPrgMap->FileHandle));
      } break;
      case TTACID: {
        u32 CodeBufSize;
        u8 *pCodeBuf;
        
        fread(&CodeBufSize,1,4,(FILE*)(pPrgMap->FileHandle));
        ...
        fread(&SourceSamplesCount,1,4,(FILE*)(pPrgMap->FileHandle));
        
        fread(pCodeBuf,1,CodeBufSize,(FILE*)(pPrgMap->FileHandle));
