...
__attribute__((noinline)) bool Shell_FindShellPath_FindIns(char *pBasePath)
{
  ...
  
  {
    ...
    snprintf(fn,PathnameMaxLength,"%s/codecomposer.ini",pBasePath);
...
{
  strcpy(FATShellPath,"/codecomposer");
  strcpy(FATShellSkinPath,"/codecomposer/skin");
  strcpy(FATShellPluginPath,"/codecomposer/plugin");
  strcpy(FATShellCustomPath,"/codecomposer/custom");
  FATShellCustomLangPath[0]=0;
  
  {
    if(FAT_CWD("/codecomposer")==true){
      FAT_FILE *pf=FAT_fopen("/codecomposer/codecomposer.ini","r");
      if(pf!=NULL){
        FAT_fclose(pf);
  ...
  iprintf("can not found codecomposer folder.\n");
