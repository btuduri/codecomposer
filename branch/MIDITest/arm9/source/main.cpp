...
//---------------------------------------------------------------------------------
	...
	FILE* file_handle;
	file_handle = (FILE*)Shell_OpenFile("Dancing_Queen.mid");
...
bool Start(FILE* FileHandle)
{
 ...
 PCH_SetProgramMap((FILE*)GetBINFileHandle());
 
 {
   bool detect=false;
   
   if(strncmp((char*)DeflateBuf,"MThd",4)==0){
     iprintf("Start Standard MIDI file.\n");
     detect=true;
   }
   
   if(detect==false){
     iprintf("Unknown file format!!\n");
     PCH_FreeProgramMap();
     if(DeflateBuf!=NULL){
       safefree(DeflateBuf); DeflateBuf=NULL;
     }
