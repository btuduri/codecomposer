...
u16 getRTCtoFileTime (void)
{
	...
	*/
	return (
		( ( (IPC->time.rtc.hours > 11 ? IPC->time.rtc.hours - 40 : IPC->time.rtc.hours) & 0x1F) << 11) |
		( (IPC->time.rtc.minutes & 0x3F) << 5) |
		( (IPC->time.rtc.seconds >> 1) & 0x1F) );
#else
	return 0;
#endif
}
...
u16 getRTCtoFileDate (void)
{
	...
	*/
	return ( 
		( ((IPC->time.rtc.year + 20) & 0x7F) <<9) |
		( (IPC->time.rtc.month & 0xF) << 5) |
		(IPC->time.rtc.day & 0x1F) );
#else
	return 0;
#endif
}
...
bool FAT_InitFiles (void)
{
	...
	else	// This is an MBR
	{
		...
		iprintf("MBR boot sector=%d ",bootSector);
	}
...
*/
	...
	iprintf("RD=%d ",filesysRootDir);
	iprintf("RE=%d ",bootSec->rootEntries);
	iprintf("REsize=%d\n",sizeof(DIR_ENT));
	iprintf("SPC=%d ",bootSec->sectorsPerCluster);
	iprintf("FD=%d ",filesysData);
	iprintf("FLC=%d\n",fatLastCluster);

	if (fatLastCluster < 4085)
	{
		iprintf("Detect FAT12\n",0);
		filesysType = FS_FAT12;	// FAT12 volume - unsupported
	}
	else if (fatLastCluster < 65525)
	{
		iprintf("Detect FAT16\n",0);
		filesysType = FS_FAT16;	// FAT16 volume
	}
	else
	{
		iprintf("Detect FAT32\n",0);
	...
	{
		lfnName[i] = '\0';
#include "initfiles1.h"
	}

#ifdef UseFatDump
#include "initfiles2.h"
#endif
...
#include "getlongfilenameunicode.h"
